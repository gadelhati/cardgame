...
- Each player drags one card at a time, from the source field to the destination field
    - from a field origin: deck, hand, desk or trash;
    - to another field, destination: deck, hand, desk or trash or to a card.
        - skills can be activated when drag or when drop;
        - in a specific field
- If the card has an effect, that effect will have an origin, a destination, and a consequence
- The turn passes when the card is droped

## Card construction
- Border: The card features a clean outer border (padding) colored according to its archetype.
    - Archetype Color(azul, verde, ...): The archetype's color changes during card creation and is fixed after creation, and may change according to buffs or nerfs, the card may have up to two archetypes.
- Top Section:
